...
public class MovieCategoryBusiness {
    ...
    public ResponseEntity<ResponseModelMsg> findById(String categoryId) {
        ...
        if (movieCategory != null) {
            return new ResponseEntity<>(new ResponseModelMsg("Registro encontrado.", 1, movieCategory), HttpStatus.OK);
        }
    ...
    public ResponseEntity<ResponseModelMsg> save(MovieCategory movieCategory) {
        try {
            ...
            if (movieCategory.getDescryption() == null || movieCategory.getDescryption().isEmpty()) {
                return new ResponseEntity<>(new ResponseModelMsg("Descrição da categoria deve ser preechido", 5), HttpStatus.BAD_REQUEST);
            }
            baseRepository.getMovieCategories().add(movieCategory);
            return new ResponseEntity<>(new ResponseModelMsg("Registro salvo com sucesso.", 3, movieCategory), HttpStatus.CREATED);
        } catch (Exception e) {
    ...
    public ResponseEntity<ResponseModelMsg> update(MovieCategory movieCategory) {
        try {
            if (movieCategory.getId() == null || movieCategory.getId().isEmpty()) {
                return new ResponseEntity<>(new ResponseModelMsg("Id da categoria deve ser preechido", 4), HttpStatus.BAD_REQUEST);
            }

            if (HttpStatus.OK != findById(movieCategory.getId()).getStatusCode()) {
                return new ResponseEntity<>(new ResponseModelMsg("Id da categoria deve exitir", 4), HttpStatus.BAD_REQUEST);
            }
